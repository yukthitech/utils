...
public class _ObjectBuilderSettingEvaluator {
    ...
    private void setJavaBeanProperties(Object bean,
        ...
        try {
        	List<BeanProperty> writeableProp = BeanProperty.loadProperties(cl, false, true);
        	beanPropSetters = new HashMap(writeableProp.size() * 4 / 3, 1.0f);
        	
        	for(BeanProperty prop : writeableProp)
        	{
        		beanPropSetters.put(prop.getName(), prop.getWriteMethod());
        	}
        } catch (Exception e) {
