...
/**
 * Used to invoke Multi part PUT REST API.
 * @author akiran
 */
...
public class InvokePutWithAttachmentStep extends AbstractRestStep implements Validateable
{
	...
	public void execute(AutomationContext context, ExecutionLogger exeLogger) throws Exception
	{
		...
		for(Map.Entry<String, Object> partEntry : parts.entrySet())
		{
			...
			{
				putRestRequest.addTextPart(partEntry.getKey(), (String) partEntry.getValue());
			}
			else
			{
				putRestRequest.addJsonPart(partEntry.getKey(), partEntry.getValue());
			}
		}
		...
		for(Map.Entry<String, String> attachEntry : attachments.entrySet())
		{
			...
			putRestRequest.addAttachment(attachEntry.getKey(), file, null);
		}
